...
class LingqCollectionsWorker(QWorkerBase):
    ...
    def get_lingq_courses(self):
        ...
        networker = NetworkWorker(
            "GET",
            f"https://www.lingq.com/api/v3/zh/collections/my/?page={self.page}&page_size=10&search_criteria=contains&sort=recentUsed&username={keys["lingqusername"]}",
        )
    ...
    def lingq_courses_response(self, res: NetworkResponse):
        if res.ok:
            ...
            if "results" in result and len(result["results"]) > 0:
                ...
                self.logging(f"Received Page: {self.page} of Lingq Courses")
                print(result)
