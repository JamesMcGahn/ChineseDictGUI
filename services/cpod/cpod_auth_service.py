...
from . import CpodNewLoginWorker
from .cpod_token_manager import CpodTokenManager
...
class CpodAuthService(BaseAuthService):
    ...
    def validate(self):
        ...
        if self.session.has_token:
            has_valid_token = self.token_manager.check_token(self.session.token)

        return AuthValidationResponse(
            cookies_valid=has_valid_cookies, token_valid=has_valid_token
        )
    ...
    def ensure_authenticated(self):
        ...
        # fall back
        if self.session.has_token and not result.token_valid:
            self.logging("Token expired. Refreshing token.")
            self._busy = True
    ...
    def token_status(self, auth_status: AUTHVALIDATIONSTATUS):
        self._busy = False
        self.logging(f"Token status: {auth_status}")
    ...
    def login(self):
        ...
        login_worker = CpodNewLoginWorker(session=self.session)
        login_thread = QThread()
        login_worker.moveToThread(login_thread)
        login_worker.cpod_logged_in.connect(self.receive_login)
